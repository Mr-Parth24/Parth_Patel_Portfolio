...
import { ArrowDown, Sparkles, GraduationCap, Briefcase, FileText, Download } from 'lucide-react';
...
export default function HeroSection() {
  ...
  return (
    ...
    >
      {/* Parallax glows */}
      <motion.div className="pointer-events-none absolute inset-0" style={{ x: springX, y: springY }}>
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-6 lg:px-12">
        <div className="flex flex-col items-center gap-12 lg:flex-row lg:items-center lg:justify-between lg:gap-16">
          ...
          >
            ...
            {/* Typewriter */}
            <motion.div variants={item} className="mb-4 flex h-9 items-center justify-center lg:justify-start">
              <p className="text-lg font-semibold text-text-secondary sm:text-xl">
                <span>{displayed}</span>
          ...
          >
            ...
            <div className="absolute inset-0 scale-125 rounded-full blur-3xl opacity-20"
              style={{ background: 'radial-gradient(circle, #10B981 0%, #3B82F6 50%, transparent 70%)' }} />

            {/* Slow-spinning dashed ring */}
            <div className="absolute inset-0 rounded-full border-2 border-dashed border-accent-emerald/25 scale-110"
              style={{ animation: 'spin-slow 18s linear infinite' }} />

            {/* Second ring — opposite direction */}
            <div className="absolute inset-0 rounded-full border border-accent-blue/20 scale-[1.18]"
              style={{ animation: 'spin-slow 28s linear infinite reverse' }} />

            {/* Solid glowing border */}
            <div className="absolute -inset-1 rounded-full"
              style={{ background: 'linear-gradient(135deg, #10B981, #3B82F6, #8B5CF6, #10B981)', backgroundSize: '300% 300%', animation: 'gradient-shift 4s ease infinite', padding: 3, borderRadius: '50%' }}>
            ...
            {/* Small dot accents */}
            <div className="absolute top-4 -left-2 h-3 w-3 rounded-full bg-accent-emerald opacity-70"
              style={{ animation: 'glow-pulse 2.5s ease-in-out infinite' }} />
            <div className="absolute bottom-8 -right-3 h-2 w-2 rounded-full bg-accent-violet opacity-60"
              style={{ animation: 'glow-pulse 3s ease-in-out infinite 1s' }} />
            <div className="absolute -bottom-1 left-8 h-2.5 w-2.5 rounded-full bg-accent-blue opacity-50"
